<!-- select2 from array -->
@php
    $svg = \SVG\SVG::fromFile(resource_path($field['svg_path']));
    $field['options'] =  collect($svg->getDocument()->getElementsByTagName('symbol'))
        ...
        ->pluck('id', 'id');
@endphp
<div style="display: none">{{ require(resource_path($field['svg_path'])) }}</div>
            ...
            if (!element.hasClass("select2-hidden-accessible"))
                {
                    element.select2({
                        ...
                        templateResult: function(data) {
                            function tmp(svg) {
                                return '<svg style="width: 40px; height: 40px; margin-right: 10px">\n' +
                                    '    <use xlink:href="#'+svg+'" />\n' +
                                    '</svg>';
                            }
                            return tmp(data.id) + data.text;
                        },
